...
interface ServiceLayerError {
  error: string
  detailErrors: Array<DetailedError> | undefined
  statusCode: number
}
...
export abstract class BaseServiceException extends Error implements ServiceLayerError {
  error: string
  statusCode: number
  detailErrors: Array<DetailedError>
  constructor(error: string, detailedErrors: Array<DetailedError>) {
    super(error)
    this.error = error
...
export class BadRequestError extends BaseServiceException {
  constructor(error: string, detailedErrors: Array<DetailedError> = null) {
